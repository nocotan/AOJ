...
// 素数表の生成
void make_primes(std::vector<int>& v, int n) {
    v.assign(n, 0);
    std::iota(v.begin(), v.end(), 2);
    for(auto d=v.begin(); (*d)*(*d)<=v.back(); ++d)
        v.erase(remove_if(d+1, v.end(), [&](int x){ return x%*d==0; }), v.end());
}

vector<int> primes;

int solve(int n, int p) {
    int k = upper_bound(ALL(primes), n) - primes.begin();
    vector<int> m;
    for(int i=k; i<k+p; ++i)
        for(int j=k; j<=i; ++j) m.push_back(primes[i] + primes[j]);
    sort(ALL(m));
    return m[p-1];
}
...
int main()
{
    make_primes(primes, 120000);
    int n, p;
    while(cin>>n>>p, n!=-1) cout << solve(n, p) << endl;
    return 0;
}
