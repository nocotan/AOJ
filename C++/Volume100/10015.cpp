...
int main() {
  int n;
  cin >> n;
  vi s(13,0), h(13,0), c(13,0), d(13,0);
  rep(i,n) {
    string mark;
    int num;
    cin >> mark >> num;
  ...
  rep(i,s.size()) if(s[i]==0) printf("S %d\n", i+1);
  rep(i,h.size()) if(h[i]==0) printf("H %d\n", i+1);
  rep(i,c.size()) if(c[i]==0) printf("C %d\n", i+1);
  rep(i,d.size()) if(d[i]==0) printf("D %d\n", i+1);
  return 0;
}
